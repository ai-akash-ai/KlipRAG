...
def format_shopping_results_for_llm(products: List[Dict[str, Any]]) -> str:
    ...
    for i, product in enumerate(products, 1):
        ...
        formatted_results += f"   Store: {product.get('source', 'N/A')}\n"
...
async def get_retrieval_chain(user_id: str, query: str):
    ...
    if intent == "personal":
        ...
        When providing recommendations, use the retailer data to suggest where users can buy items. Mention that "another user has purchased [item] at [retailer name], [amount], and [date]"."""
        ...
        if intent == "recommendation":
            ...
            if not exact_item_found:
                ...
                if shopping_results:
                    ...
                    The exact item was not found in our retailer database, so here are current shopping options. Include prices, ratings, store names, and direct links from the shopping results. Be specific about where users can buy items online."""

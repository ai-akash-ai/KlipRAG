...
try:
    ...
    if response.status_code == 200:
        ...
        if api_status.get("model_loaded"):
            st.success("Connected to transcription API with Whisper model loaded")
        else:
            st.warning("Connected to API but Whisper model is not loaded correctly")
    else:
...
if st.session_state.recording and not st.session_state.audio_filename:
    ...
    try:
        ...
        if audio_chunks:
            ...
            status_placeholder.info("Sending audio to API for transcription...")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("success"):
                        ...
                        status_placeholder.error(f"API error: {result.get('message')}")
                else:
                    status_placeholder.error(f"API returned status code {response.status_code}")
            except Exception as e:
                status_placeholder.error(f"Error sending audio to API: {e}")
        else:
...
st.markdown("---")
st.markdown("This app uses OpenAI's Whisper for transcription via FastAPI backend.")
